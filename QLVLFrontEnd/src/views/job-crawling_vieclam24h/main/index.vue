...
import { crawlJobUrlsVieclam24h, crawlJobInfoVieclam24h, stopCrawling } from "@/services/user.service";
...
const toggleCrawlUrl = async () => {
  ...
  startCrawling('urls', crawlJobUrlsVieclam24h);
};
...
const toggleCrawlData = async () => {
  ...
  startCrawling('info', crawlJobInfoVieclam24h);
};
...
const stopCrawlingProcess = async () => {
  try {
    ...
    const scriptKey = currentCrawlType.value === 'urls' 
      ? 'crawl-job-urls-vieclam24h' 
      : 'crawl-job-info-vieclam24h';
