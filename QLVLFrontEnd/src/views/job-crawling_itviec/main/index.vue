...
import { crawlJobUrlsItviec, crawlJobInfoItviec, stopCrawling } from "@/services/user.service";
...
const toggleCrawlUrl = async () => {
  ...
  startCrawling('urls', crawlJobUrlsItviec);
};
...
const toggleCrawlData = async () => {
  ...
  startCrawling('info', crawlJobInfoItviec);
};
...
const stopCrawlingProcess = async () => {
  try {
    ...
    const scriptKey = currentCrawlType.value === 'urls' 
      ? 'crawl-job-urls-itviec' 
      : 'crawl-job-info-itviec';
