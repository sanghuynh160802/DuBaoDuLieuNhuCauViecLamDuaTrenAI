...
class ModelHandler:
    ...
    def predict_future(self, city_name, job_name, start_date, steps=36):
        ...
        current_window_scaled = self.scaler.transform(self.data)

        if start_date > last_historical_date:
            print("\n[Mode] Future prediction only (no historical overlap)")

            current_window = current_window_scaled[-LOOK_BACK:].copy()
            ...
            predictions = []
            dates = []
            for i in range(LOOK_BACK, len(current_window_scaled)):
                X_input = current_window_scaled[i - LOOK_BACK:i].reshape(1, LOOK_BACK, len(self.columns))
            ...
            if remaining_steps > 0:
                last_known_index = len(current_window_scaled) - LOOK_BACK
                X_input = current_window_scaled[last_known_index:last_known_index + LOOK_BACK].reshape(1, LOOK_BACK, len(self.columns))
